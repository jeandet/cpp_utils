...
TEST_CASE("Endianness vector", "[decode]")
{
    ...
    REQUIRE_THAT(output, Catch::Equals<uint8_t>({2, 1, 4,3,6,5,8,7,10,9,12,11,14,13,16,15}));
    decode<target_endianness>(input.data(),std::size(input),reinterpret_cast<uint32_t*>(output.data()));
    REQUIRE_THAT(output, Catch::Equals<uint8_t>({4,3,2,1,8,7,6,5,12,11,10,9,16,15,14,13}));
    decode<target_endianness>(input.data(),std::size(input),reinterpret_cast<uint64_t*>(output.data()));
    REQUIRE_THAT(output, Catch::Equals<uint8_t>({8,7,6,5,4,3,2,1,16,15,14,13,12,11,10,9}));
}
