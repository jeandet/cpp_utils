...
#define HAS_METHOD(method)                                                     \
  ...
  template<class T>                                                            \
  static inline constexpr bool has_##method##_method = details::_has_##method##_method<T>::value;
...
#define HAS_MEMBER(member)                                                     \
  ...
  template<class T>                                                            \
  static inline constexpr bool has_##member##_member_object = details::_has_##member##_member_object<T>::value;


namespace
  cpp_utils::types_detectors
{

  template < typename T, typename = void >struct is_qt_tree_item:
    std::false_type
  {
  };

  template < typename T > struct is_qt_tree_item <
    T,
  decltype (std::declval < T > ().takeChildren (),
  ...
  template < class T > static inline constexpr bool
    is_qt_tree_item_v = is_qt_tree_item < T >::value;
